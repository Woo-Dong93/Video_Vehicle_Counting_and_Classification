...
#define RINFO_MAX    332 // 화면 사이즈(Y)
#define RINFO_X      11 // 동일차량 x좌표 이동값
#define RINFO_Y      11 // 동일차량 y좌표 이동값
#define RINFO_VALID  345 // 더이상 카운트하지 않는 범위(y)
...
int	main(int argc, char *argv[]) {
	...
	VideoCapture capture("video2.mp4"); // 공주대 정문 육교
	...
	for (;;)
	{
		...
		/////////////////// 직선 그리기
		crossingLine[0].x = 330;
		crossingLine[0].y = 332;
		crossingLine[1].x = 720;
		crossingLine[1].y = 332;
		...
		threshold(sub_frame, imgThresh, 30, 255, CV_THRESH_BINARY); // Threshold -> 잡음제거
		...
		for (int j = 1; j < numOfLables; j++)
		{
			...
			if (width < 40 || height < 50) // 작은 사각형들은 필터링(제거)
	        continue;
			if (area < 6730 ) // 작은 사각형들 필터링(제거)
			continue;

			

			if (x < 320 || x > 710) // x는 센터값 => 일정범위에서는 필터링(제거)
			{
				continue;
			}

        	if (y < 332 || y > 345) // y는 센터값 => 일정범위에서는 필터링(제거)
    		{
	    	  continue;
		    }
